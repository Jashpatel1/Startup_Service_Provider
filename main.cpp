	...
	while(1)
	{
       	 	...
       	 	if(s == "company")
        	{
                ...
                system(s);
                    ...
                    fprintf(fp,"%s %s %s %s %s %s %s %s %d\n",B.firstName.c_str(),B.lastName.c_str(),B.emailid.c_str(),B.contact.c_str(),B.established.c_str(),B.cityofestablishment.c_str(),B.state.c_str(),B.interest_tag.c_str(),B.votes);
                    fclose(fp);
                    break;
        	}
        	if(s == "startup-user")

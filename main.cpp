...
class startupUser{
    ...
    void set_uid()
    {
        ...
        cin>>uid;
        FILE* fp;
        fp = fopen("/src/database/startupdatabase.txt","r+");
        char c_uid[25],c_pass[25];
        fscanf(fp,"%d",&n);
        for(i=0;i<n;i++)
        {
	...
	void check_pass_startup()
        {
            ...
            char cpass[25];
            fp = fopen("src/database/startupdatabase.txt","r");
            fpl = fopen("src/database/startupregdetails.txt","r");
            int n;
            fscanf(fp,"%d",&n);
            int i;
            for(i=0;i<n;i++)
	...
	while(1)
	{
       	 	...
       	 	if(s == "company")
        	{
                ...
                system(s);
            		...
            		FILE *fpl;
            		fp = fopen("src/database/companydatabase.txt","r+");
                    fpl = fopen("src/database/companyregdetails.txt","r+");
                    int qw;
            		fscanf(fp,"%d",&qw);
            		fseek(fp,0,SEEK_SET);
            		...
            		fclose(fp);
            		fclose(fpl);
            		fp = fopen("src/database/companydatabase.txt","a");
            		fprintf(fp,"%s %s\n",B.uid.c_str(),B.password.c_str());
            		fclose(fp);
                    fp = fopen("src/database/companyregdetails.txt","a");
        	...
        	else if(s == "startup-user")
        	{
                ...
                system(s);
            		...
            		fprintf(fpl,"%d\n",n+1);
            		fclose(fp);
            		fp = fopen("src/database/startupdatabase.txt","a");
           	 	    fprintf(fp,"%s %s\n",A.uid.c_str(),A.password.c_str());
            		fclose(fp);
                    fp = fopen("src/database/startupregdetails.txt","a");
